...
class HourglassNet(nn.Module):
    ...
    def __init__(self, block, num_stacks=2, num_blocks=4, num_classes=16, args=None):
    ...
    def get_loss(self, logits, gt_labels):
        ...
        if type(logits) == list:  # multiple logits
            loss_val = 0
            for o in logits:
                loss_val += self.loss_cross_entropy(o, gt) 
    ...
    def erase_feature_maps(self, atten_map_normed, feature_maps, threshold=0.6):
    ...
    def get_localization_maps(self):
        ...
        return map1#torch.max(map1, map_erase)
        # return map_erase
# dummy = torch.ones((2, 3, 64, 64))

# block = HourglassNet(Bottleneck, 4, 1, 10)
# print len(block(dummy))
#print(block) 
