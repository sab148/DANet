...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=1000, threshold=0.6):
        ...
        self.cls_erase2 = self.classifier(512, num_classes)
    ...
    def forward(self, x, label=None):
        ...
        out3 = self.get_localization_maps2()
        localization_map_normed2 = self.get_atten_map(out3, label, True)
        
        self.attention2 = localization_map_normed2
        localization_map_normed2 = 1 - localization_map_normed2
        feat_erase2 = self.erase_feature_maps(localization_map_normed2, x, self.threshold)
        
        # Branch C 
        out_erase2 = self.cls_erase2(feat_erase2)
        self.map_erase2 = out_erase2.view(batch_size, self.num_classes, 28, 28)
        logits_ers2 = torch.mean(torch.mean(out_erase2, dim=2), dim=2)

        return [logits_1, logits_ers, logits_ers2]
    ...
    def get_loss(self, logits, gt_labels):
        ...
        loss_cls_ers2 = self.loss_cross_entropy(logits0[2], gt_labels.long())

        loss_val = loss_cls + loss_cls_ers + loss_cls_ers2

        return loss_val



    def get_localization_maps2(self):
    ...
    def get_localization_maps(self):
        map1 = self.normalize_atten_maps(self.cam_map)
        map_erase = self.normalize_atten_maps(self.map_erase)
        map_erase2 = self.normalize_atten_maps(self.map_erase2)
        return torch.max(torch.max(map1, map_erase),map_erase2)
...
model = VGG(make_layers(cfg['D1'], dilation=dilation['D1']))
print model
