...
class VGG(nn.Module):
    ...
    def forward(self, x):
        ...
        parentResult_multimaps = self.cls4(parentResult).view(batch_size, 37, self.num_maps, 14, 14)
        parentResult = torch.sum(parentResult_multimaps, 2).view(batch_size, 37, 14, 14)/self.num_maps
        ...
        childResult = torch.sum(childResult_multimaps, 2).view(batch_size, self.num_classes, 14, 14)/self.num_maps
    ...
    def get_loss(self, logits, gt_root_label, gt_parent_label, gt_child_label):
        ...
        maps = torch.cat((
            F.upsample((self.child_map.reshape(batch_size*self.num_classes, self.num_maps, 14, 14)[[gt_child_label[i].long()+(i*self.num_classes) for i in range(batch_size)], :, :, :]).reshape(batch_size, self.num_maps, 14, 14),size=(28, 28), mode='bilinear', align_corners=True),
            F.upsample((self.parent_map.reshape(batch_size * 37, self.num_maps, 14, 14)[[gt_parent_label[i].long() + (i * 37) for i in range(batch_size)], :, :, :]).reshape(batch_size, self.num_maps, 14, 14), size=(28, 28),
