...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=1000):
        ...
        self.cls1 = self.classifier(512)
        self.fc8 = nn.Conv2d(1024, num_classes, kernel_size=1, padding=0)
        
        self.cls2 = self.classifier(1024)
        self.fc9 = nn.Conv2d(1024, num_classes, kernel_size=1, padding=0)
        
        self.cls3 = self.classifier(1024)
        self.fc10 = nn.Conv2d(1024, num_classes, kernel_size=1, padding=0)

        self._initialize_weights()

        self.loss_cross_entropy = nn.CrossEntropyLoss()

    def classifier(self, in_planes):
    ...
    def forward(self, x, label=None):
        ...
        out = self.fc8(x)
        batch_size = x.size(0)
        self.cam_map1 = out.view(batch_size, self.num_classes, 14, 14)
        logits0 = torch.mean(torch.mean(self.cam_map1, dim=2), dim=2)

        x = self.cls2(x)        
        out = self.fc9(x)
        self.cam_map2 = out.view(batch_size, self.num_classes, 14, 14)
        logits1 = torch.mean(torch.mean(self.cam_map2, dim=2), dim=2)

        x = self.cls3(x)
        out = self.fc10(x)        
        self.cam_map3 = out.view(batch_size, self.num_classes, 14, 14)
    ...
    def get_loss(self, logits, gt_labels):
        ...
        loss_cls1 = self.loss_cross_entropy(logits0[0], gt_labels.long())
        loss_cls2 = self.loss_cross_entropy(logits0[1], gt_labels.long())
        loss_cls3 = self.loss_cross_entropy(logits0[2], gt_labels.long())


        loss_val = loss_cls1 + loss_cls2 + loss_cls3

        return loss_val


    def get_cam_maps(self):
        return torch.mean(torch.stack([self.cam_map1, self.cam_map2, self.cam_map3]),dim=0)

        #return torch.max(torch.max(self.cam_map1, self.cam_map2),self.cam_map3)
