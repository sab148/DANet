...
from utils.localization import get_top_cam
...
train_list = os.path.abspath(os.path.join(ROOT_DIR, '../data', 'CUB_200_2011/list/test.txt'))
...
def val(args):
    ...
    thresholds = [0.15] #list(thresholds)
    
    for th in thresholds:

        top1_locerr.reset()
        top5_locerr.reset()
        for idx, dat in tqdm(enumerate(train_loader)):
            ...
            top_boxes = get_top_cam(pred_prob[idx, :], cam_map, img_path[0], args.input_size,
                                                 args.crop_size, topk=(1, ), threshold=th, mode='union')
